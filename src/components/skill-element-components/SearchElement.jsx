...
export default function SearchElement({
  ...
  const elementText =
    "Custom search" + (searchLists.length > 1 ? ` ${idx + 1}` : "");
  ...
  const searchElementChildren =
    searchTermList.length > 0 ? searchTermList : defaultSearchTerm;
  ...
  return (
    <div className="SearchElement" id={skillId}>
      ...
      >
        <span className="SkillElementImageContainer">
          <LazyLoadImage
            ...
            alt={elementText}
            src={`./skill-icon-small.png`}
          />
        </span>
        <span className="SkillElementText">
          <span className="SkillElementTextSpan">{elementText}</span>
      ...
      <SubSkillDropDownContainer
        display={display}
        id={skillId}
        children={searchElementChildren}
      />
    </div>
  );
}
