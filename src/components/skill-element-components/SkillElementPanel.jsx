...
export default function SkillElementPanel({
  skillElementObject,
  setSkillElementObject,
  searchLists,
  setSearchLists,
  SliderPanelComponent,
  toggleDisplayTutorial,
}) {
  ...
  function createToggleAll(skillId, selectAllBool) {
    function toggleAll() {
      ...
      newSkillElementObject[skillId].deselectedSubSkills = !selectAllBool
        ? [...initialSkills[skillId].deselectedSubSkills]
        : [];
      newSkillElementObject[skillId].selectedSubSkills = selectAllBool
        ? [...initialSkills[skillId].deselectedSubSkills]
        : [];
  ...
  function createToggleSubSkill(skillId, subSkillName, selectBool) {
    function toggleSubSkill() {
      const newSkillElementObject = structuredClone(skillElementObject);
      const deselectedSubSkills =
        newSkillElementObject[skillId].deselectedSubSkills;
      const selectedSubSkills =
        newSkillElementObject[skillId].selectedSubSkills;
  ...
  function objectToSkillList(skillElementObject, selectedBool) {
    ...
    Object.keys(skillElementObject).map((skillId) => {
      ...
      const areSelectedSkills =
        skill.selectedSubSkills.length > 0 && selectedBool;
      const aredeselectedSkills =
        skill.deselectedSubSkills.length > 0 && !selectedBool;

      //If nothing relevant is selected, don't render this element
      if (!(areSelectedSkills || aredeselectedSkills)) return;

      skillElementList.push(
        <SkillElement
          key={skillId}
          skillId={skillId}
          name={skill.name}
          onClick={createToggleAll(skillId, !selectedBool)}
          skillElementObject={skillElementObject}
          createToggleSubSkill={createToggleSubSkill}
  ...
  const selectedChildren = searchElements.concat(
    objectToSkillList(skillElementObject, true),
  );
  ...
  return (
    <span className="SkillElementPanel" style={{ width }}>
      <div
        className="SlideTab"
        id="slide-tab-tutorial"
        onClick={toggleDisplayTutorial}
      >
        <div className="SlideTabText">?</div>
      </div>
      <div
        className="SlideTab"
        id="slide-tab-skills"
