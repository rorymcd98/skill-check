...
export default function SkillFrequencyChart({ chartData }) {
  ...
  const transition = selected
    ? "opacity 0.4s cubic-bezier(.91,-0.01,.91,.4)"
    : "opacity 0.4s cubic-bezier(0,.8,.18,1)";
  ...
  const summaryChart = (
    <Bar
      ...
      options={options}
      data={data}
      style={{ position: "relative", opacity, zIndex, transition }}
    />
  );
  ...
  return (
    <div className="SkillFrequencyChart">
      <button
        className="SkillFrequencyChartArrow"
        id="left-arrow"
        onClick={prevChart}
      />
      <button
      ...
      />
      <span
        className="BarComponentContainer"
